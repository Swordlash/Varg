...
layoutWords = ["let", "where", "has", "with", "matching"]
      ...
      | isLayout t0 =
        case ts
            ...
            -- will push an explicit layout block.
              of
          t1:_
            | isLayoutOpen t1 -> moveAlong st [t0] ts
          ...
          _ ->
            let col =
                ...
                st' = Implicit col : st
                    -- Do we have to insert an extra layoutSep?
             in case st of
                  Implicit n:_
                    | newLine pt t0 &&
                        column t0 == n && not (isNothing pt || isTokenIn [layoutSep, layoutOpen] (fromJust pt)) ->
                      let b':t0':b'':ts'' = addToken (afterPrev pt) layoutSep (t0 : b : ts')
        ...
        let st' = drop 1 (dropWhile isImplicit st)
         in if null st'
              then error $
                   "Layout error: Found " ++
                   layoutClose ++
                   " at (" ++ show (line t0) ++ "," ++ show (column t0) ++ ") without an explicit layout block."
